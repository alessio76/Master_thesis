...
namespace uclv{
    ...
    std::vector<Eigen::Isometry3f> build_scene(tf2_ros::StaticTransformBroadcaster& static_broadcaster, bool simulation, const std::string& object_frame_name, 
       ...
       while(tfBuffer.canTransform(out.str(), base_frame_name, ros::Time(0),ros::Duration(1))){
            ...
            object_to_attach.header.frame_id = base_frame_name;
    ...
    bool call_plan_service(std::vector<std::tuple<moveit_msgs::RobotTrajectory, std::string, std::string>>& full_traj, const Eigen::Isometry3f& target_pose,
                        ...
                        const std::string& move_gripper="", bool homing=false, bool attach=false, bool detach=false){
        ...
        geometry_msgs::TransformStamped goal_pose = uclv::eigen_to_TransformedStamped(base_frame_name, goal_frame_name, trans, quat);
        ...
        plan_service.request.attach = attach;
        plan_service.request.detach = detach;
    ...
    void read_gripper_speed(const sun_wsg50_common::Status::ConstPtr& msg) {
        
        current_gripper_speed = msg -> speed;
    }
    ...
    bool move_gripper(float gripper_width, bool simulation, ros::Publisher& joint_pub, const std::string& gripper_command, ros::NodeHandle& nh, 
                      float max_gripper_width, float gripping_force, float gripper_requested_speed){
        static int n=0;
        bool success=true;
        ...
        if(simulation){
            ...
            //divide by 2 and by 1000 because gripper width is in mm and because in simulation the joint is mirrored by the other finger, 
            //so the distance is the half
            if(gripper_command == "close"){
                joint_cmd.position.push_back(-(max_gripper_width - gripper_width)/(2*1e3));
        ...
        //real gripper
            ...
            if(gripper_command == "close"){
                ...
                set_service.request.val = gripping_force;
                ...
                if (force_gripper_client.call(set_service)){
                    ...
                    close_service.request.speed = gripper_requested_speed;
            ...
            if(success == true){
                ros::Subscriber wait_gripper_stop = nh.subscribe<sun_wsg50_common::Status>("/status", 1, read_gripper_speed);

                //wait until gripper stops
                while(std::abs(current_gripper_speed) > 1e-5){
                    ROS_INFO_STREAM(current_gripper_speed);
                    ros::spinOnce();
                }
...
int main(int argc, char** argv){
    ...
    std::vector<Eigen::Vector3f> pre_place_positions;
    int count=0;
    pose_name << "target_pose" << count;

    while(n.hasParam(pose_name.str())){
        std::vector<float> temp;
        n.getParam(pose_name.str(), temp);
        pre_place_positions.push_back(Eigen::Vector3f(temp[0],temp[1],temp[2]));
        pose_name.str("");
        pose_name.clear();
        count++;
        pose_name << "target_pose" << count;
    ...
    ros::Subscriber jointStateSub = n.subscribe(joint_topic, 1, uclv::get_current_state);
    ...
    for(int j=0; j < obj_pose.size(); j++){
        ...
        pre_grasp_service.request.start_state = initial_state;
        ...
        if(pre_grasp_client.call(pre_grasp_service)){   
            ...
            if(pre_grasp_service.response.success){
                ...
                if(uclv::call_plan_service(full_traj, grasp_pose, plan_service, "cartesian", ARM, start_state, static_broadcaster, plan_client, "","close")){
                    ...
                    if(uclv::call_plan_service(full_traj, post_grasp_pose, plan_service, "cartesian", ARM, start_state, static_broadcaster, plan_client, out.str(), "", false, true, false)){
                        //uclv::askContinue("POST GRASP TRAJECTORY?. Continue?");
                        //move on the same height of the post_grasp pose
                        pre_place_positions[j][2] = post_grasp_pose.translation()[2];
                        pre_place_pose = Eigen::Isometry3f(Eigen::Translation3f(pre_place_positions[j]) * Eigen::Quaternionf(post_grasp_pose.rotation()));
                        ...
                        if(uclv::call_plan_service(full_traj, pre_place_pose, plan_service, "joint", ARM, start_state, static_broadcaster, plan_client, out.str())){
                            ...
                            if(uclv::call_plan_service(full_traj, place_pose, plan_service, "cartesian", ARM, start_state, static_broadcaster, plan_client, "", "open")){
                                ...
                                if(uclv::call_plan_service(full_traj, post_place_pose, plan_service, "cartesian", ARM, start_state, static_broadcaster, plan_client, out.str(),"",false, false, true)){
