...
class Mask_to_image:
	...
	#extract every image in the data and make the full mask
	def rearrange_image(self,img_data,h,w):
		count=0
		img=np.zeros((h,w)).astype(np.uint8)

		while count<len(img_data):
			elem=img_data[count:count+h*w].reshape((h,w))
			img+=elem.astype(np.uint8)
			count+= h*w
	...
	# Reshape the received data into a 2D numpy array
		...
		image_data=self.rearrange_image(image_data, self.height, self.width)
